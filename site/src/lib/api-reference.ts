  ...
  pptx: [
    {
      ...
      options: [
        ...
        { name: 'enableElementSelection', type: 'boolean', def: 'false', desc: 'Make slide-element clicks establish read-only element context; no mutation or editor model is exposed.' },
        { name: 'elementHitTolerance', type: 'number', def: '6', desc: 'Straight-line and connector hit tolerance in CSS pixels for element context clicks.' },
        { name: 'onSelectionContextChange', type: '(context: PptxSelectionContext | null) => void', desc: 'Receive bounded detached text-selection or element-click context for AI/MCP handoff. Text selection takes precedence.' },
    ...
    },
    {
      ...
      methods: [
        ...
        { sig: 'presentSlide(canvas, index, opts?: { width?, dpr?, onTextRun? }): Promise<PresentationHandle>', desc: 'Render a slide and attach canvas-native audio/video playback, returning a handle with play() / pause() / destroy(). Works in both modes — in `mode: "worker"` the base slide and text-run geometry are produced off-thread and the video overlay is composited on the main thread.' },
        ...
        { sig: 'getElementContextAt(slideIndex, point, options?): Promise<PptxElementSelectionContext | null>', desc: 'Return compact context for the topmost transformed element frame at a slide-EMU point in either mode (line segments use tolerance). Includes master/layout/slide provenance, never editor tree indexes or mutable elements.' },
        RESOURCE_METRICS_METHOD,
    ...
    },
    {
      ...
      options: [
        ...
        { name: 'enableElementSelection', type: 'boolean', def: 'false', desc: 'Make clicks on mounted slide canvases establish read-only element context.' },
        { name: 'elementHitTolerance', type: 'number', def: '6', desc: 'Straight-line and connector hit tolerance in CSS pixels.' },
        { name: 'onSelectionContextChange', type: '(context: PptxSelectionContext | null) => void', desc: 'Receive bounded detached text or element context for external AI/MCP integrations.' },
        FIND_HIGHLIGHT_COLORS,
  ...
  docx: [
    {
      ...
      options: [
        ...
        { name: 'onSelectionContextChange', type: '(context: DocxSelectionContext | null) => void', desc: 'Receive bounded detached selected text and page/paragraph/run locators for AI/MCP handoff.' },
        FIND_HIGHLIGHT_COLORS,
      ...
      methods: [
        ...
        { sig: 'get canvasElement(): HTMLCanvasElement', desc: 'The underlying canvas.' },
        { sig: 'getSelectionContext(options?: DocxSelectionContextOptions): DocxSelectionContext | null', desc: 'Return the current bounded, JSON-serializable native text-selection snapshot. Throws after destroy().' },
        RESOURCE_METRICS_METHOD,
        { sig: 'destroy(): void', desc: 'Tear down the worker and release resources.' },
      ],
    },
    ...
    },
    {
      ...
      options: [
        ...
        { name: 'onSelectionContextChange', type: '(context: DocxSelectionContext | null) => void', desc: 'Receive bounded detached selected text and source locators for external AI/MCP integrations.' },
        FIND_HIGHLIGHT_COLORS,
        ON_HYPERLINK_CLICK,
        ENABLE_HYPERLINKS,
        { name: 'showTrackChanges', type: 'boolean', def: 'true', desc: 'Intended to show revision markup for tracked insertions and deletions using author colours, insertion underlines and deletion strikethroughs. The current retained paint pipeline does not apply this flag; true and false render identically.', emphasis: 'The current retained paint pipeline does not apply this flag; true and false render identically.' },
      ...
      methods: [
        ...
        { sig: 'getSelectionContext(options?: DocxSelectionContextOptions): DocxSelectionContext | null', desc: 'Return the current mounted native text-selection context.' },
        RESOURCE_METRICS_METHOD,
        { sig: 'destroy(): void', desc: 'Tear down the DOM subtree. Destroys a self-loaded engine; a borrowed one is left intact.' },
      ],
    },
  ...
  xlsx: [
    {
      ...
      options: [
        ...
        { name: 'onSelectionChange', type: '(sel: CellRange | null) => void', desc: 'Deprecated lossy active-area projection. Migrate to onSelectionStateChange before removal in 0.77.0.' },
      ...
      methods: [
        ...
        { sig: 'getSelectionContext(options?: { maxCells?: number; maxTextCharacters?: number }): XlsxSelectionContext | null', desc: 'Return `{ format: "xlsx", kind: "range" }` cell-count- and text-bounded context for read-only AI/MCP handoff, including formulas and Viewer-formatted display text.' },
        ...
        { sig: 'get selection(): CellRange | null', desc: 'Deprecated lossy active-area projection; removed in 0.77.0.' },
        { sig: 'select(ref: string): void', desc: 'Deprecated compatibility alias for setSelection; removed in 0.77.0.' },
    ...
    },
    {
      ...
      options: [
        ...
        { name: 'onSelectionChange', type: '(sel: CellRange | null) => void', desc: 'Deprecated lossy active-area projection. Removed in 0.77.0.' },
        VIEWER_ON_ERROR,
      ],
      methods: [
        ...
        { sig: 'getSelectionContext(options?: { maxCells?: number; maxTextCharacters?: number }): XlsxSelectionContext | null', desc: 'Return `{ format: "xlsx", kind: "range" }` cell-count- and text-bounded detached context for AI/MCP use.' },
        { sig: 'copySelection(): Promise<XlsxCopyResult>', desc: 'Copy bounded TSV and return an observable result.' },
        { sig: 'get selection(): CellRange | null', desc: 'Deprecated lossy active-area projection; removed in 0.77.0.' },
        { sig: 'select(ref: string): void', desc: 'Deprecated compatibility alias; removed in 0.77.0.' },
        { sig: 'getCellAt(clientX: number, clientY: number): CellAddress | null', desc: 'Hit-test a viewport coordinate to a cell address.' },
