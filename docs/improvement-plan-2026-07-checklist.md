...
## Phase 4 — 大型構造リファクタ ✅ 全完了（2026-07-03/04、PR #681, #684–#697, #699–#700）
...
- [x] B2 後続 = **B2 完結（#693, #697, #700）**: table T1 (#693) = セル測定を paint 側 `measureCellContentHeightPx` に一本化（empty-mark/ruby の潜在乖離 2 件も修正、VRT 0/19 変化）+ テーブル stamp/reuse。textbox (#697) = 簡易エンジン（wrapShapeText/wrapShapeRuns）削除 → 本体エンジン統一で **kinsoku/bidi/justify/tab が textbox で発火**（B5 吸収。8 サンプル中 6 byte-identical、sample-10 は Word と同一行数に改善、sample-15 の 1 語乖離は #698 へ切り出し）。物理分割 (#700) = line-layout エンジン 57 ブロック（2,219 行）を `line-layout.ts` へ verbatim 移動（renderer.ts 9,924→7,777 行、3 重証明: ブロック byte 同一 + stay 再構成 0 mismatch + スクリーンショット 19 枚 diff exit 0）+ `__test_*` 5 個を正規 export 化（B9/B10、6 個は意図的残存）。B8 は LayoutLine privacy 境界の設計理由で見送り（flat stamp フィールド維持）。B2 T2（セル内段落 stamp）は将来候補
- [x] 参照更新 (#699): demo VRT 参照を現世代へ再生成（ユーザー承認）。**発見: 旧参照は canvas 高丸めが 1 世代古く（842px vs 正しい 841px = round(841.9 × 595/595.3)）、visual.spec.ts の size-mismatch pad 吸収で不可視だった** — 正常化済み、通常 VRT 21/21 安定
...
- [x] E9: **案 A（現状維持）採用 — ユーザー確定（2026-07-03）**。調査結論: フォント同梱でも macOS CoreText と Linux FreeType の rasterizer 差で pixel-perfect 可搬化は原理的に不可。VRT はローカル専用（private 100% カバー）を維持し、CI の回帰検出は E1 smoke が担う。案 B（Noto 同梱 + demo のみ CI-VRT、検出力 7-29%）/ 案 C（折衷）は不採用
...
- [x] B12 帰結 — **3 ヒューリスティックとも Word ground-truth で完全解消（fixture 合成 → ユーザー Word PDF → pdftotext bbox 照合のループを確立、sample-17〜22 の 6 fixture / 55+ 観測ケース）**:
  - **#675 ✅ #690**: frame keep-with-anchor（§17.3.1.11 は寸法のみ規定）。Word PDF と完全一致検証済み（vAnchor='text' は frame+anchor を次ページへ、'page' は据え置き）
  - **#674 ✅ #691→#694**: #691 の「丸ごと relocate」は Word 実測で**誤りと判明し reopen** → #694 で**行単位 greedy 分割**（複数ページ continuation band、anchor は終端ページ本文 top から flow — sample-18/21 と 0.1pt 一致）+ **page/margin-anchored の物理ページ内 clamp**（実測 741.9 = 841.9 − 100、frame にも同修正）に置換
  - **#676 ✅ #696**: 1-em 閾値は Word 規則でないと実測で確定 — **真の規則 = 「gap ≥ 72pt（ちょうど 1 インチ）で行を float 横に開始、内容・フォントサイズ・行間に非依存。qualifying gap では語を強制折り返し」**（40 ケース全一致、70pt→below/72pt→beside 境界含む）。empty 1-em / 第一トークン幅の 2 系統判定を単一 grounded rule に一本化（tolerance 0.05pt = 1 twip は授与粒度由来）
- [x] #698（新規起票 → 実測で再スコープ）: textbox 統一で表面化した SPACE_SHRINK_RATIO=0.25 の 2 症状（sample-10 タイトル / sample-15 justified 1 語）は、計装実測により**単一ノブでは解けないと証明**（per-glyph k 区間が空、justified ゲートは両方に逆効果、89 発火サイトの大半が正当な justified 本文）。**0.25 は実測上の最適運用点として維持**。残余は (a) centered text-box のグリフクリップ（advance 系統差、フォント代替依存の可能性）と (b) justified over-pack（Word に圧縮 fit があるかの bracket fixture で判別可能）として issue に記録、open 維持
