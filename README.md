...
- **[`packages/node/`](packages/node/)** — Node-side parsers (`@silurus/ooxml-node`) exposing materializing `parsePptx` / `parseDocx` / `parseXlsx` / `parseXlsxAllSheets` helpers plus bounded `openPptxPresentation`, `openDocxDocument`, and `iterateXlsxWorksheetRows` sessions against the workspace WASM artifacts, with no DOM or Web Worker dependency. `openDocxDocument` completes the format-required sequential pagination once, then renders caller-owned page canvases one at a time through the canonical layout pipeline. Useful for CI checks, headless rendering pipelines, and CLI tools. Includes an `ooxml-thumbnail` CLI (pptx-only first pass; requires `skia-canvas`).
